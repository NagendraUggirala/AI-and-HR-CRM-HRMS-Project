...
const [newStructure, setNewStructure] = useState({
  ...
  try {
    ...
    autoTable(doc, {
      startY: 65,
      head: [['Name', 'Grade', 'Level', 'Category', 'CTC', 'Status', 'Employees', 'Effective Date', 'Created By', 'Version']],
      body: tableData,
      theme: 'striped',
...
const handleExportAssignmentsPDF = () => {
  try {
    ...
    // Summary Statistics
    const activeAssignments = assignments.filter(a => a.assignmentStatus === 'active').length;
    const pendingAssignments = assignments.filter(a => a.assignmentStatus === 'pending').length;
    ...
    autoTable(doc, {
      startY: yPosition,
      head: [['Employee ID', 'Name', 'Department', 'Grade', 'Structure', 'Allocation', 'CTC', 'Take-Home', 'Effective Date', 'Status']],
      body: tableData,
      theme: 'striped',
      headStyles: { 
      ...
      didDrawPage: (data) => {
        ...
        doc.text("Confidential - Structure Assignments Report", 14, doc.internal.pageSize.height - 10);
      }
    });
...
const handleDownloadAssignmentPDF = (assignment) => {
  try {
    ...
    yPosition += 7;
    doc.text(`Employee Type: ${assignment.employeeType || "N/A"}`, 14, yPosition);
    yPosition += 10;
    ...
    doc.setFontSize(10);
    doc.text(`Current Structure: ${assignment.currentStructure || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Grade: ${assignment.grade || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Level: ${assignment.level || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Assignment Type: ${assignment.assignmentType || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Assignment Status: ${assignment.assignmentStatus || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Effective Date: ${assignment.effectiveDate || "N/A"}`, 14, yPosition);
    yPosition += 7;
    ...
    doc.setFontSize(10);
    doc.text(`Annual CTC: ₹${(assignment.ctc || 0).toLocaleString()}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Monthly Take-Home: ₹${(assignment.takeHome || 0).toLocaleString()}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Monthly Gross Salary: ₹${(assignment.grossSalary || 0).toLocaleString()}`, 14, yPosition);
    
    if (assignment.annualTakeHome) {
      yPosition += 7;
      doc.text(`Annual Take-Home: ₹${assignment.annualTakeHome.toLocaleString()}`, 14, yPosition);
    }
    
    if (assignment.previousCTC) {
      yPosition += 7;
      doc.text(`Previous CTC: ₹${assignment.previousCTC.toLocaleString()}`, 14, yPosition);
    }
    yPosition += 10;
    ...
    if (assignment.allocationRules) {
      ...
      doc.setFontSize(10);
      doc.text(`Type: ${assignment.allocationRules.type || "N/A"}`, 14, yPosition);
      yPosition += 7;
      doc.text(`Basis: ${assignment.allocationRules.basis || "N/A"}`, 14, yPosition);
      yPosition += 7;
      doc.text(`Pro-rata: ${assignment.allocationRules.proRata ? "Yes" : "No"}`, 14, yPosition);
      yPosition += 7;
      doc.text(`Custom Rules: ${assignment.allocationRules.hasCustomRules ? "Yes" : "No"}`, 14, yPosition);
      yPosition += 10;
    }
...
const handleExportSingleStructurePDF = (structure) => {
  try {
    ...
    yPosition += 7;
    doc.text(`Grade: ${structure.grade} | Level: ${structure.level || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Category: ${structure.category || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Employee Type: ${structure.employeeType || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Annual CTC: ₹${(structure.ctc || 0).toLocaleString()}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Effective Date: ${structure.effectiveDate || "N/A"}`, 14, yPosition);
    yPosition += 7;
    ...
    doc.setFontSize(10);
    doc.text(`Departments: ${(structure.department || []).join(", ") || "N/A"}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Locations: ${(structure.location || []).join(", ") || "N/A"}`, 14, yPosition);
    yPosition += 10;
    ...
    doc.setFontSize(10);
    doc.text(`Minimum CTC: ₹${(structure.minCTC || 0).toLocaleString()}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Maximum CTC: ₹${(structure.maxCTC || 0).toLocaleString()}`, 14, yPosition);
    yPosition += 7;
    doc.text(`Increment Range: ${structure.incrementRange || "N/A"}`, 14, yPosition);
    yPosition += 10;
    ...
    if (structure.standardDeductions?.pf?.enabled) {
      doc.text(`PF: ${structure.standardDeductions.pf.employee}% (Employee), ${structure.standardDeductions.pf.employer}% (Employer)`, 14, yPosition);
      yPosition += 7;
    }
    if (structure.standardDeductions?.esi?.enabled) {
      doc.text(`ESI: ${structure.standardDeductions.esi.employee}% (Employee), ${structure.standardDeductions.esi.employer}% (Employer)`, 14, yPosition);
      yPosition += 7;
    }
    if (structure.standardDeductions?.professionalTax?.enabled) {
      doc.text(`Professional Tax: ₹${structure.standardDeductions.professionalTax.amount}`, 14, yPosition);
      yPosition += 7;
    }
    if (structure.standardDeductions?.tds?.enabled) {
      doc.text(`TDS: ${structure.standardDeductions.tds.slab || "Enabled"}`, 14, yPosition);
      yPosition += 7;
    }
    yPosition += 10;

    // Employee Count
    doc.text(`Assigned Employees: ${structure.employeeCount || 0}`, 14, yPosition);
    yPosition += 10;
...
const handleExportSingleStructureExcel = (structure) => {
  try {
    ...
    const structureData = [
      ...
      ["Level", structure.level || "N/A"],
      ["Category", structure.category || "N/A"],
      ["Sub Category", structure.subCategory || "N/A"],
      ["Employee Type", structure.employeeType || "N/A"],
      ["Annual CTC", `₹${(structure.ctc || 0).toLocaleString()}`],
      ["Min CTC", `₹${(structure.minCTC || 0).toLocaleString()}`],
      ["Max CTC", `₹${(structure.maxCTC || 0).toLocaleString()}`],
      ["Increment Range", structure.incrementRange || "N/A"],
      ["Status", structure.status || "N/A"],
      ["Approval Status", structure.approvalStatus || "N/A"],
      ["Effective Date", structure.effectiveDate || "N/A"],
      ["Expiry Date", structure.expiryDate || "N/A"],
      ["Employee Count", structure.employeeCount || 0],
      ["Created On", structure.createdAt || "N/A"],
      ["Created By", structure.createdBy || "N/A"],
      ["Version", structure.version || "v1.0"],
      ["Departments", (structure.department || []).join(", ") || "N/A"],
      ["Locations", (structure.location || []).join(", ") || "N/A"],
      ["Description", structure.description || ""],
      ["Change Reason", structure.changeReason || ""],
    ];
    ...
    const deductionsData = [
      ...
      [
        ...
        "N/A",
        "N/A",
        structure.standardDeductions?.professionalTax?.amount ? `₹${structure.standardDeductions.professionalTax.amount}` : "N/A",
        "N/A"
      ],
      [
        "TDS",
            ...
            <div className="col-12 mb-3">
              ...
              <div className="form-check form-check-inline">
                <input
                  ...
                  onChange={handleInputChange}
                />
                <label className="form-check-label" htmlFor="statutory">
                  Statutory
                </label>
              </div>
              <div className="form-check form-check-inline">
              ...
              <div className="form-check form-check-inline">
                <input
                  ...
                  onChange={handleInputChange}
                />
                <label className="form-check-label" htmlFor="requiresProof">
                  Requires Proof
                </label>
              </div>
              <div className="form-check form-check-inline">
            ...
            <div className="col-12 mb-3">
              <div className="form-check form-check-inline">
                <input
                  ...
                  onChange={handleInputChange}
                />
                <label className="form-check-label" htmlFor="edit-taxable">
                  Taxable
                </label>
              </div>
              <div className="form-check form-check-inline">
                <input
                  ...
                  onChange={handleInputChange}
                />
                <label className="form-check-label" htmlFor="edit-statutory">
                  Statutory
                </label>
              </div>
              <div className="form-check form-check-inline">
                <input
                  ...
                  onChange={handleInputChange}
                />
                <label className="form-check-label" htmlFor="edit-proRata">
                  Pro-rata Calculation
                </label>
              </div>
              <div className="form-check form-check-inline">
  ...
  <div className="row g-3 align-items-center">
    ...
    <div className="col-12 col-md">
      <div className="d-flex flex-column flex-sm-row gap-2 justify-content-md-end">
        ...
        {/* Add Component */}
        <button
          className="btn btn-light border text-muted d-flex align-items-center justify-content-center"
        ...
        <div className="dropdown">
          <button
            className="btn btn-light border text-muted dropdown-toggle d-flex align-items-center justify-content-center w-100"
            type="button"
...
<div className="card bg-light mb-4">
  <div className="card-body">
    <h6 className="mb-3">🔍 Filter Components</h6>
    ...
    <div className="row g-3">
      ...
      <div className="col-12 col-sm-6 col-md-4 col-lg-3">
        ...
        <select
          ...
          onChange={(e) =>
            setComponentFilters({ ...componentFilters, category: e.target.value })
          }
        >
      ...
      <div className="col-12 col-sm-6 col-md-4 col-lg-3">
        ...
        <select
          ...
          onChange={(e) =>
            setComponentFilters({ ...componentFilters, type: e.target.value })
          }
        >
      ...
      <div className="col-12 col-sm-6 col-md-4 col-lg-3">
        ...
        <select
          ...
          onChange={(e) =>
            setComponentFilters({ ...componentFilters, taxable: e.target.value })
          }
        >
      ...
      <div className="col-12 col-sm-6 col-md-4 col-lg-3">
        ...
        <select
          ...
          onChange={(e) =>
            setComponentFilters({ ...componentFilters, statutory: e.target.value })
          }
        >
          <option value="all">All</option>
      ...
      <div className="col-12 col-sm-6 col-md-4 col-lg-3">
        ...
        <select
          ...
          onChange={(e) =>
            setComponentFilters({ ...componentFilters, calculation: e.target.value })
          }
        >
      ...
      <div className="col-12 col-sm-6 col-md-4 col-lg-3">
        ...
        <select
          ...
          onChange={(e) =>
            setComponentFilters({ ...componentFilters, proRata: e.target.value })
          }
        >
          <option value="all">All</option>
      ...
      <div className="col-12 col-sm-6 col-md-4 col-lg-3">
        ...
        <select
          ...
          onChange={(e) =>
            setComponentFilters({ ...componentFilters, status: e.target.value })
          }
        >
...
<div className="mb-5">
  <div className="d-flex justify-content-between align-items-center mb-3">
    ...
    className="btn btn-sm btn-danger text-light"
        onClick={() => handleExportCategoryPDF('deductions', filteredDeductions)}
  ...
  <div className="col-lg-6 mb-4">
    <div className="d-flex justify-content-between align-items-center mb-3">
      ...
      <button
        ...
        onClick={() => handleExportCategoryPDF('employer_contributions', filteredEmployerContributions)}
          ...
          <div className="d-flex flex-column flex-md-row justify-content-between gap-2">
            ...
            <div className="flex-grow-1">
              ...
              <div className="d-flex gap-3 flex-wrap">
                ...
                <span className="small">
                  <strong>Pro-rata:</strong> {comp.proRata ? "Yes" : "No"}
                </span>
                <span className="small">
                  <strong>Rounding:</strong> {getRoundingBadge(comp.rounding)}
                </span>
              </div>
    ...
    <div className="d-flex justify-content-between align-items-center mb-3">
      ...
      <button
       className="btn btn-sm btn-success text-light"
        onClick={() => handleExportCategoryPDF('reimbursements', filteredReimbursements)}
      ...
      {filteredReimbursements.map((comp) => (
        ...
        >
            ...
            <div className="flex-grow-1">
              ...
              {comp.taxExemptLimit && (
                <div className="small text-success mt-1">
                  Tax exempt up to ₹{comp.taxExemptLimit.toLocaleString()}
                </div>
              )}
            </div>
          ...
          <div className="card bg-light">
            <div className="card-body">
              <h6 className="mb-3">📊 Component Statistics</h6>
              <div className="row">
...
{showMenu && (
  <div
    className="position-absolute end-0 mt-1 bg-white border rounded shadow-lg z-3"
  ...
  >
    <div className="p-2">
      ...
      <div className="small text-muted px-2 mb-1">Download</div>
...
<div className="d-flex flex-column flex-sm-row gap-2 align-items-stretch">
  ...
  <button
    className="btn btn-light border text-muted d-flex align-items-center justify-content-center w-100 w-sm-auto"
    type="button"
...
<div className="card bg-light mb-4">
  <div className="card-body">
    <h6 className="mb-3">🔍 Filter Structures</h6>
            ...
            <div className="card-body d-flex flex-column">
              ...
              <div className="mb-3">
                ...
                <div className="d-flex justify-content-between">
                  <span className="text-muted small">CTC vs Take-Home</span>
...
<div className="d-flex flex-column flex-md-row align-items-start align-items-md-center justify-content-between mb-4 gap-3">
  ...
  <div className="d-flex flex-column flex-sm-row gap-2">
    ...
    <button
      className="btn btn-primary border text-light d-flex align-items-center justify-content-center w-100 w-sm-auto"
  ...
  <div className="table-responsive">
    <table className="table table-hover align-middle">
      <thead className="table-light">
        <tr>
          <th width="10%" className="ps-3">Employee ID</th>
          <th width="15%">Name</th>
          ...
          <th width="4%" className="pe-3">Actions</th>
        </tr>
      </thead>
      <tbody>
          ...
          <tr key={assignment.id}>
            ...
            <td>
              ...
              {assignment.assignmentStatus && (
                <div className="mt-1">
                  <span className={`badge small ${assignment.assignmentStatus === 'active' ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning'}`}>
                    {assignment.assignmentStatus}
                  </span>
                </div>
              )}
              ...
              <div className="d-flex flex-column gap-1">
                ...
                {assignment.allocationRules?.type === "auto" ? (
                  <span className="badge bg-success-subtle text-success border small d-flex align-items-center">
                    <Settings size={10} className="me-1 flex-shrink-0" />
            ...
            <td>
              ...
              {assignment.assignmentType && (
                <div className="small text-muted">
                  ...
                  {assignment.assignmentType === "revision" && "Revision"}
                  {assignment.assignmentType === "promotion" && "Promotion"}
                  {assignment.assignmentType === "transfer" && "Transfer"}
                </div>
              )}
            </td>
            <td>
            ...
            <td>
              <div className="d-flex flex-column">
                ...
                {assignment.createdOn && (
                  <div className="small text-muted">
                    Created: {assignment.createdOn.split('-').reverse().join('/')}
                  </div>
                )}
              </div>
            </td>
<td className="pe-3">
  <div className="d-flex gap-1">
    ...
    <button
      className="btn btn-sm btn-outline-success"
      onClick={() => handleDownloadAssignmentPDF(assignment)}
        ...
        >
          <div className="card-body p-4">
            <div className="d-flex align-items-center justify-content-between mb-3">
              <div>
                <h5 className="mb-1">
                  <Icon icon="heroicons:currency-dollar" /> Salary Structure
                  Management
                </h5>
                <p className="text-muted small mb-0">
...
<div className="row g-3 mb-4">
  {/* Card 1: Total Structures */}
  <div className="col-md-4 col-sm-6 col-12">
    <div className="card h-100 border-0 shadow-sm bg-primary bg-opacity-10">
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-center">
          <div>
            <h6 className="text-dark mb-1 small fw-semibold text-uppercase">
              Total Structures
            </h6>
            <h3 className="text-dark mb-1">{structures.length}</h3>
          </div>
        ...
        <div className="mt-3">
          <div className="row g-2">
            <div className="col">
              <div className="d-flex align-items-center">
                <CheckCircle size={12} className="text-dark me-1" />
                <span className="text-dark small">Active:</span>
              </div>
            ...
            <div className="col">
              <div className="d-flex align-items-center">
                <Edit2 size={12} className="ttext-dark me-1" />
                <span className="text-dark small">Draft:</span>
              </div>
              <div className="text-dark fw-medium">
                {structures.filter((s) => s.status === "draft").length}
              </div>
      ...
      <div className="card-body">
        <div className="d-flex justify-content-between align-items-center">
          <div>
            ...
            <h3 className="text-dark mb-1">
              {assignments.filter((a) => a.assignmentStatus === "active").length}
            </h3>
          </div>
        ...
        <div className="mt-3">
          <div className="row g-2">
            <div className="col">
              <div className="d-flex align-items-center">
                <Users size={12} className="text-dark me-1" />
                <span className="text-dark small">Total:</span>
              </div>
              <div className="text-dark fw-medium">{assignments.length}</div>
            </div>
            <div className="col">
              <div className="d-flex align-items-center">
                <Clock size={12} className="text-dark me-1" />
                <span className="text-dark small">Pending:</span>
              </div>
              <div className="text-dark fw-medium">
      ...
      <div className="card-body">
        ...
        <div className="mt-3">
          <div className="row g-2">
            ...
            <div className="col">
              <div className="d-flex align-items-center">
                <XCircle size={12} className="text-dark me-1" />
                <span className="text-dark small">Inactive:</span>
              </div>
              <div className="text-dark fw-medium">
                {Object.values(components)
                  .flat()
          ...
          <div className="row">
            ...
            <div className="col-12 mb-4">
              ...
              <div className="row">
                ...
                <div className="col-md-4 mb-3">
                  <label className="form-label">Increment Range (%)</label>
                  <input
                    type="text"
                    className="form-control"
              ...
              <div className="row">
                <div className="col-md-6 mb-3">
                  ...
                  <div className="bg-light p-3 rounded" style={{ maxHeight: "150px", overflowY: "auto" }}>
                    {["Engineering", "Product", "Design", "Marketing", "Sales", "Operations", "Finance", "HR", "IT"].map((dept) => (
                      <div key={dept} className="form-check mb-2">
                        <input
                          className="form-check-input"
                          type="checkbox"
                          id={`dept-${dept}`}
                          checked={newStructure.departments?.includes(dept) || false}
                          onChange={(e) => {
                ...
                <div className="col-md-6 mb-3">
                  ...
                  <div className="bg-light p-3 rounded" style={{ maxHeight: "150px", overflowY: "auto" }}>
                    {["Bengaluru", "Hyderabad", "Pune", "Mumbai", "Delhi", "Chennai", "Remote", "International"].map((location) => (
                      <div key={location} className="form-check mb-2">
                        <input
                          className="form-check-input"
                          type="checkbox"
                          id={`loc-${location}`}
                          checked={newStructure.locations?.includes(location) || false}
                          onChange={(e) => {
              ...
              <div className="row">
                <div className="col-md-6 mb-3">
                  ...
                  <input
                    type="date"
                    className="form-control"
                    value={newStructure.effectiveDate || new Date().toISOString().split("T")[0]}
                    onChange={(e) =>
                      setNewStructure({
                        ...newStructure,
              ...
              <div className="row">
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch mb-3">
                    ...
                    />
                    <label className="form-check-label fw-medium" htmlFor="pfEnabled">
                      Provident Fund (PF)
                    </label>
                  </div>
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch mb-3">
                    ...
                    />
                    <label className="form-check-label fw-medium" htmlFor="esiEnabled">
                      ESI Contribution
                    </label>
                  </div>
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch mb-3">
                    ...
                    />
                    <label className="form-check-label fw-medium" htmlFor="ptEnabled">
                      Professional Tax
                    </label>
                  </div>
                  {newStructure.ptEnabled !== false && (
                    <div className="ms-3">
                      <label className="form-label small">Monthly Amount (₹)</label>
                      <input
                        type="number"
                        className="form-control form-control-sm"
                        placeholder="200"
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch">
                    ...
                    />
                    <label className="form-check-label fw-medium" htmlFor="tdsEnabled">
                      TDS Deduction
                    </label>
                  </div>
        ...
        <div className="modal-footer">
          ...
          <button
            ...
            onClick={() => {
              ...
              if (!newStructure.name || !newStructure.grade || !newStructure.ctc || !newStructure.level || !newStructure.category) {
                setActionNotification({
                  show: true,
                  type: "error",
              ...
              const newStruct = {
                ...
                approvalStatus: newStructure.status === "active" ? "approved" : "pending",
                employeeCount: 0,
                version: "v1.0",
                parentVersion: null,
                ...
                componentGroups: {
                  earnings: newStructure.selectedComponents?.filter(id => 
                    components.earnings.some(c => c.id === id)
                  ) || [1, 2, 3, 4],
                  deductions: newStructure.selectedComponents?.filter(id => 
                    components.deductions.some(c => c.id === id)
                  ) || [6, 8],
                  employerContributions: newStructure.selectedComponents?.filter(id => 
                    components.employerContributions.some(c => c.id === id)
                  ) || [12],
                  reimbursements: newStructure.selectedComponents?.filter(id => 
                ...
                standardDeductions: {
                  ...
                  esi: { 
                    ...
                    employee: parseFloat(newStructure.esiEmployee || 0.75),
                    employer: parseFloat(newStructure.esiEmployer || 3.25)
                  },
                  professionalTax: { 
                ...
                // Salary Bands
                minCTC: parseInt(newStructure.minCTC || newStructure.ctc * 0.8),
                maxCTC: parseInt(newStructure.maxCTC || newStructure.ctc * 1.2),
                incrementRange: newStructure.incrementRange || getIncrementRangeFromGrade(newStructure.grade),
      ...
      <div className="modal-content">
                    ...
                    {["Engineering", "Product", "Design", "Marketing", "Sales", "Operations", "Finance", "HR", "IT"].map((dept) => (
                      <div key={dept} className="form-check mb-2">
                        <input
                          ...
                          type="checkbox"
                          id={`edit-dept-${dept}`}
                          checked={selectedStructure.department?.includes(dept) || false}
                          onChange={(e) => {
                            const currentDepartments = selectedStructure.department || [];
                            if (e.target.checked) {
                              setSelectedStructure({
                                ...selectedStructure,
                              ...
                              setSelectedStructure({
                                ...selectedStructure,
                                department: currentDepartments.filter(d => d !== dept)
                              });
                            }
                          }}
                        />
                        <label className="form-check-label" htmlFor={`edit-dept-${dept}`}>
                          {dept}
                        </label>
                      </div>
                    ))}
                    ...
                    {["Bengaluru", "Hyderabad", "Pune", "Mumbai", "Delhi", "Chennai", "Remote", "International"].map((location) => (
                      <div key={location} className="form-check mb-2">
                        <input
                          ...
                          checked={selectedStructure.location?.includes(location) || false}
                          onChange={(e) => {
                            const currentLocations = selectedStructure.location || [];
                            if (e.target.checked) {
                              setSelectedStructure({
                                ...selectedStructure,
                        ...
                        />
                        <label className="form-check-label" htmlFor={`edit-loc-${location}`}>
                          {location}
                        </label>
                      </div>
                    ))}
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch mb-3">
                    <input
                      ...
                      type="checkbox"
                      id="edit-pfEnabled"
                      checked={selectedStructure.standardDeductions?.pf?.enabled !== false}
                      onChange={(e) =>
                        setSelectedStructure({
                          ...selectedStructure,
                          standardDeductions: {
                    ...
                    />
                    <label className="form-check-label fw-medium" htmlFor="edit-pfEnabled">
                      Provident Fund (PF)
                    </label>
                  </div>
                  {selectedStructure.standardDeductions?.pf?.enabled !== false && (
                    <div className="row ms-3">
                      <div className="col-6">
                        <label className="form-label small">Employee %</label>
                        <input
                          type="number"
                          className="form-control form-control-sm"
                          value={selectedStructure.standardDeductions?.pf?.employee || "12"}
                          onChange={(e) =>
                            setSelectedStructure({
                              ...selectedStructure,
                              standardDeductions: {
                      ...
                      <div className="col-6">
                        ...
                        <input
                          type="number"
                          className="form-control form-control-sm"
                          value={selectedStructure.standardDeductions?.pf?.employer || "12"}
                          onChange={(e) =>
                            setSelectedStructure({
                              ...selectedStructure,
                              standardDeductions: {
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch mb-3">
                    <input
                      ...
                      type="checkbox"
                      id="edit-esiEnabled"
                      checked={selectedStructure.standardDeductions?.esi?.enabled || false}
                      onChange={(e) =>
                        setSelectedStructure({
                          ...selectedStructure,
                          standardDeductions: {
                    ...
                    />
                    <label className="form-check-label fw-medium" htmlFor="edit-esiEnabled">
                      ESI Contribution
                    </label>
                  </div>
                    ...
                    <div className="row ms-3">
                      <div className="col-6">
                        ...
                        <input
                          ...
                          step="0.01"
                          className="form-control form-control-sm"
                          value={selectedStructure.standardDeductions?.esi?.employee || "0.75"}
                          onChange={(e) =>
                            setSelectedStructure({
                              ...selectedStructure,
                              standardDeductions: {
                      ...
                      <div className="col-6">
                        ...
                        <input
                          ...
                          step="0.01"
                          className="form-control form-control-sm"
                          value={selectedStructure.standardDeductions?.esi?.employer || "3.25"}
                          onChange={(e) =>
                            setSelectedStructure({
                              ...selectedStructure,
                              standardDeductions: {
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch mb-3">
                    <input
                      ...
                      type="checkbox"
                      id="edit-ptEnabled"
                      checked={selectedStructure.standardDeductions?.professionalTax?.enabled !== false}
                      onChange={(e) =>
                        setSelectedStructure({
                          ...selectedStructure,
                          standardDeductions: {
                    ...
                    />
                    <label className="form-check-label fw-medium" htmlFor="edit-ptEnabled">
                      Professional Tax
                    </label>
                  </div>
                  {selectedStructure.standardDeductions?.professionalTax?.enabled !== false && (
                    <div className="ms-3">
                      <label className="form-label small">Monthly Amount (₹)</label>
                      <input
                        type="number"
                        className="form-control form-control-sm"
                        value={selectedStructure.standardDeductions?.professionalTax?.amount || "200"}
                        onChange={(e) =>
                          setSelectedStructure({
                            ...selectedStructure,
                            standardDeductions: {
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch">
                    <input
                      ...
                      type="checkbox"
                      id="edit-tdsEnabled"
                      checked={selectedStructure.standardDeductions?.tds?.enabled || false}
                      onChange={(e) =>
                        setSelectedStructure({
                          ...selectedStructure,
                          standardDeductions: {
                    ...
                    />
                    <label className="form-check-label fw-medium" htmlFor="edit-tdsEnabled">
                      TDS Deduction
                    </label>
                  </div>
                  {selectedStructure.standardDeductions?.tds?.enabled && (
                    <div className="ms-3">
                      ...
                      <select
                        className="form-select form-select-sm"
                        value={selectedStructure.standardDeductions?.tds?.slab || "as_per_income"}
                        onChange={(e) =>
                          setSelectedStructure({
                            ...selectedStructure,
                            standardDeductions: {
        ...
        <div className="modal-footer">
          ...
          <button
            ...
            onClick={() => {
              // Validate required fields
              if (!selectedStructure.name || !selectedStructure.grade || !selectedStructure.ctc || !selectedStructure.level || !selectedStructure.category || !selectedStructure.status) {
                setActionNotification({
                  show: true,
                  type: "error",
                  title: "Validation Error",
              ...
              const updatedStructure = {
                ...
                approvalStatus: selectedStructure.status === "active" ? "approved" : selectedStructure.approvalStatus,
                ...
                versionHistory: [
                  ...(selectedStructure.versionHistory || []),
                  {
                    ...
                    date: new Date().toISOString().split("T")[0],
                    changes: selectedStructure.changeReason || "Structure updated",
                    changedBy: "Current User",
                  }
                ]
              };
...
<div className="modal-body">
  <div className="table-responsive">
    <table className="table table-hover align-middle">
      <thead className="table-light">
        <tr>
          <th width="10%" className="ps-3">Version</th>
          <th width="25%">Structure</th>
          <th width="30%">Changes</th>
          <th width="15%">Date</th>
          <th width="15%" className="pe-3">Changed By</th>
        </tr>
      </thead>
      <tbody>
        {versions.length === 0 ? (
              ...
              <tr key={version.id}>
                <td className="ps-3">
                  <div className="d-flex flex-column align-items-start">
                    ...
                    <small className="text-muted">ID: {version.id}</small>
                  </div>
                </td>
                <td>
                  <div className="d-flex flex-column">
                ...
                <td>
                  ...
                  >
                    {version.changes}
                    {structure?.changeReason && version.id === structure.versionHistory?.[0]?.id && (
                      <div className="small text-info mt-1">
                ...
                <td>
                  <div className="d-flex flex-column">
                    <div className="d-flex align-items-center gap-1">
                      ...
                      <span className="fw-medium">{version.date}</span>
                    </div>
                    <div className="small text-muted mt-1">
                ...
                <td>
                  <div className="d-flex flex-column">
                    ...
                    {version.approver && version.approvalStatus === 'approved' && (
                      <div className="small text-success mt-1">
            ...
            <div className="col-md-6">
              <div className="card border">
                <div className="card-body">
                  ...
                  <div className="mb-2">
                    <small className="text-muted d-block">Employee Type</small>
                    <span className="fw-medium">
            ...
            <div className="col-md-6">
              <div className="card border">
                <div className="card-body">
                  ...
                  <div className="mb-2">
                    <small className="text-muted d-block">Current Structure</small>
                    <span className="fw-medium">
                  ...
                  <div className="mb-2">
                    <small className="text-muted d-block">Assignment Type</small>
                  ...
                  <div className="mb-2">
                    ...
                    <div className="mt-1">
                      <span className={`badge ${selectedAssignment.assignmentStatus === 'active' ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning'}`}>
                  ...
                  <div className="mb-2">
                    ...
                    <div className="mt-1">
                      <span className={`badge ${selectedAssignment.approvalStatus === 'approved' ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning'}`}>
              ...
              <div className="card border">
                <div className="card-body">
                  ...
                  <div className="d-flex justify-content-between mb-2">
                    ...
                    <span className="fw-bold">
                      ₹{selectedAssignment.grossSalary?.toLocaleString() || "N/A"}
                    </span>
                  </div>
                  <div className="d-flex justify-content-between mb-2">
                  ...
                  <hr />
                  <div className="d-flex justify-content-between">
                    <span className="fw-medium">CTC vs Take-Home Ratio:</span>
                    <span className="fw-bold text-info">
                      {selectedAssignment.ctc > 0 
                        ? ((selectedAssignment.takeHome * 12 / selectedAssignment.ctc) * 100).toFixed(1) + "%"
                        : "N/A"
                      }
                    </span>
                  </div>
              ...
              <div className="card border">
                <div className="card-body">
                  ...
                  <div className="mb-2">
                    ...
                    <div className="fw-medium">
                      <Calendar size={14} className="me-1 text-muted" />
                      {selectedAssignment.createdOn || selectedAssignment.assignmentDate || "N/A"}
                    </div>
                  </div>
                  <div className="mb-2">
                    ...
                    <div className="fw-medium">
                      ...
                      {selectedAssignment.lastModified || selectedAssignment.lastRevisionDate || "N/A"}
                    </div>
                  </div>
                  {selectedAssignment.contractEndDate && (
                    <div className="mb-2">
                      <small className="text-muted d-block">Contract End Date</small>
                      <div className="fw-medium">
                        <Calendar size={14} className="me-1 text-muted" />
                        {selectedAssignment.contractEndDate}
                      </div>
            ...
            <div className="col-md-6">
              <div className="card border">
                <div className="card-body">
                  ...
                  <div className="mb-2">
                    <small className="text-muted d-block">Allocation Type</small>
                    <div className="d-flex flex-wrap gap-1 mt-1">
                      {selectedAssignment.allocationRules?.type === "auto" ? (
                        <span className="badge bg-success-subtle text-success border">
                          Auto Allocation
                        </span>
                      ) : selectedAssignment.allocationRules?.type === "manual" ? (
                        <span className="badge bg-warning-subtle text-warning border">
                          Manual Allocation
                        </span>
                      ) : selectedAssignment.allocationRules?.type === "bulk" ? (
                        <span className="badge bg-info-subtle text-info border">
                          Bulk Allocation
                        </span>
                      ) : (
                      ...
                      {selectedAssignment.allocationRules?.hasCustomRules && (
                        <span className="badge bg-primary-subtle text-primary border">
                  ...
                  {selectedAssignment.allocationRules?.rules && (
                    <div className="mt-3">
                      <small className="text-muted d-block">Allocation Rules</small>
                      <div className="bg-light p-2 rounded small mt-1">
                        <ul className="mb-0 ps-3">
                          {selectedAssignment.allocationRules.rules.map((rule, index) => (
            ...
            <div className="col-md-6">
              <div className="card border">
                <div className="card-body">
                  ...
                  <div className="mb-2">
                    <small className="text-muted d-block">Change Reason</small>
                    <div className="fw-medium">
                      {selectedAssignment.changeReason || "Initial Assignment"}
                    </div>
                  </div>
                  <div className="mb-2">
                    <small className="text-muted d-block">Comments</small>
                    <div className="bg-light p-2 rounded small mt-1">
                      {selectedAssignment.comments || "No comments available"}
                    </div>
                  </div>
                  ...
                  {selectedAssignment.approvalDate && (
                    <div className="mb-2">
                      <small className="text-muted d-block">Approval Date</small>
                      <div className="fw-medium">
                        <Calendar size={14} className="me-1 text-muted" />
                        {selectedAssignment.approvalDate}
                      </div>
                ...
                <div className="row">
                  <div className="col-md-6">
                    <div className="d-flex justify-content-between mb-2">
                      <span>Basic Salary:</span>
                      <span className="fw-medium">
                        ₹{(selectedAssignment.salaryBreakdown.basic || 0).toLocaleString()}
                      </span>
                    </div>
                    <div className="d-flex justify-content-between mb-2">
                      <span>House Rent Allowance:</span>
                      <span className="fw-medium">
                        ₹{(selectedAssignment.salaryBreakdown.hra || 0).toLocaleString()}
                      </span>
                    </div>
                    <div className="d-flex justify-content-between mb-2">
                      <span>Conveyance Allowance:</span>
                      <span className="fw-medium">
                        ₹{(selectedAssignment.salaryBreakdown.conveyance || 0).toLocaleString()}
                      </span>
                    </div>
                    <div className="d-flex justify-content-between mb-2">
                      <span>Special Allowance:</span>
                      <span className="fw-medium">
                        ₹{(selectedAssignment.salaryBreakdown.specialAllowance || 0).toLocaleString()}
                      </span>
                    </div>
                  </div>
                  <div className="col-md-6">
                    {selectedAssignment.salaryBreakdown.performanceBonus && (
                      <div className="d-flex justify-content-between mb-2">
                        <span>Performance Bonus:</span>
                        <span className="fw-medium text-success">
                          ₹{(selectedAssignment.salaryBreakdown.performanceBonus || 0).toLocaleString()}
                        </span>
                      </div>
                    )}
                    {selectedAssignment.salaryBreakdown.salesCommission && (
                      <div className="d-flex justify-content-between mb-2">
                        <span>Sales Commission:</span>
                        <span className="fw-medium text-success">
                          ₹{(selectedAssignment.salaryBreakdown.salesCommission || 0).toLocaleString()}
                        </span>
                      </div>
                    )}
                    {selectedAssignment.salaryBreakdown.stockOptions && (
                      <div className="d-flex justify-content-between mb-2">
                        ...
                        <span className="fw-medium text-success">
                          ₹{(selectedAssignment.salaryBreakdown.stockOptions || 0).toLocaleString()}
                        </span>
                      </div>
                    )}
                    {selectedAssignment.salaryBreakdown.otherAllowances && (
                      <div className="d-flex justify-content-between mb-2">
                        <span>Other Allowances:</span>
                        <span className="fw-medium">
                          ₹{(selectedAssignment.salaryBreakdown.otherAllowances || 0).toLocaleString()}
                        </span>
                      </div>
                    )}
                    {selectedAssignment.salaryBreakdown.shiftAllowance && (
                      <div className="d-flex justify-content-between mb-2">
                        <span>Shift Allowance:</span>
                        <span className="fw-medium">
                          ₹{(selectedAssignment.salaryBreakdown.shiftAllowance || 0).toLocaleString()}
                        </span>
                      </div>
                    )}
                  </div>
      ...
      <div className="modal-content">
              ...
              <div className="row">
                ...
                <div className="col-md-6 mb-3">
                  ...
                  <select 
                    ...
                    onChange={(e) => setNewStructureSelection(e.target.value)}
                  >
                    <option value="">Choose a new structure (optional)...</option>
                    {structures
                ...
                <div className="col-md-6 mb-3">
                  ...
                  >
                    <option value="">Select reason (if changing)...</option>
                    <option value="promotion">Promotion</option>
            ...
            <div className="card-body">
              ...
              <div className="row">
                ...
                <div className="col-md-6 mb-3">
                  ...
                  <select 
                    className="form-select"
                    value={employeeToChange.allocationRules?.type || "auto"}
                    onChange={(e) => {
                      setEmployeeToChange({
                        ...employeeToChange,
                        allocationRules: {
                ...
                <div className="col-md-6 mb-3">
                  ...
                  <input
                    type="date"
                    className="form-control"
                    value={employeeToChange.lastModified || new Date().toISOString().split("T")[0]}
                    onChange={(e) =>
                      setEmployeeToChange({
                        ...employeeToChange,
                ...
                <div className="col-md-6 mb-3">
                  ...
                  <input
                    type="text"
                    className="form-control"
                    value={employeeToChange.lastModifiedBy || "Current User"}
                    onChange={(e) =>
                      setEmployeeToChange({
                        ...employeeToChange,
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch">
                    <input
                      ...
                      type="checkbox"
                      id="proRataEnabled"
                      checked={employeeToChange.allocationRules?.proRata || false}
                      onChange={(e) => {
                        setEmployeeToChange({
                          ...employeeToChange,
                          allocationRules: {
                    ...
                    />
                    <label className="form-check-label" htmlFor="proRataEnabled">
                ...
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch">
                    <input
                      ...
                      checked={employeeToChange.allocationRules?.hasCustomRules || false}
                      onChange={(e) => {
                        setEmployeeToChange({
                          ...employeeToChange,
                          allocationRules: {
                    ...
                    />
                    <label className="form-check-label" htmlFor="customRulesEnabled">
                      Has Custom Rules
                    </label>
                  </div>
                </div>
                ...
                {employeeToChange.allocationRules?.hasCustomRules && (
                  <div className="col-12 mb-3">
                    ...
                    <textarea
                      ...
                      value={employeeToChange.allocationRules?.rules?.join("\n") || ""}
                      onChange={(e) => {
                        setEmployeeToChange({
                          ...employeeToChange,
                          allocationRules: {
                  ...
                  <table className="table table-sm table-bordered">
                    <thead className="table-light">
                      <tr>
                        ...
                        <th width="25%" className="text-center">Current</th>
                        <th width="25%" className="text-center">New</th>
                        <th width="25%" className="text-center">Change Impact</th>
                      </tr>
                    </thead>
                    <tbody>
                      <tr>
                        ...
                        <td className="text-center">
                          {(() => {
                            const newStruct = structures.find(s => s.id.toString() === newStructureSelection);
                            return newStruct?.ctc ? `₹${newStruct.ctc.toLocaleString()}` : "N/A";
                          })()}
                        </td>
                        <td className={`text-center ${
                          (() => {
                            ...
                            const change = newCTC - (employeeToChange.ctc || 0);
                          ...
                          {(() => {
                            ...
                            const newCTC = newStruct?.ctc || 0;
                            const change = newCTC - (employeeToChange.ctc || 0);
                            if (change > 0) return `+₹${change.toLocaleString()} (Increase)`;
                            if (change < 0) return `-₹${Math.abs(change).toLocaleString()} (Decrease)`;
                            return "No Change";
                          })()}
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td className="text-center">
                          ₹{employeeToChange.takeHome?.toLocaleString() || "N/A"}
                        </td>
                        <td className="text-center">
                          {(() => {
                            const newStruct = structures.find(s => s.id.toString() === newStructureSelection);
                            const breakdown = calculateBreakdown(newStruct);
                            return breakdown?.takeHome ? `₹${breakdown.takeHome.toLocaleString()}` : "N/A";
                          })()}
                        </td>
                        <td className={`text-center ${
                          (() => {
                            ...
                            const change = newTakeHome - (employeeToChange.takeHome || 0);
                            return change > 0 ? "text-success" : change < 0 ? "text-danger" : "text-muted";
                          })()
                        }`}>
                          {(() => {
                            ...
                            const newTakeHome = breakdown?.takeHome || 0;
                            const change = newTakeHome - (employeeToChange.takeHome || 0);
                            if (change > 0) return `+₹${change.toLocaleString()}/month`;
                            if (change < 0) return `-₹${Math.abs(change).toLocaleString()}/month`;
                            return "No Change";
                          })()}
                        </td>
                      </tr>
        ...
        <div className="modal-footer">
          ...
          <button
            ...
            onClick={() => {
              // Validate required fields
              if (!employeeToChange.employeeId || !employeeToChange.name || !employeeToChange.ctc) {
                setActionNotification({
                  show: true,
                  type: "error",
                  title: "Validation Error",
                  message: "Please fill Employee ID, Name, and CTC fields.",
                });
                return;
              }

              const newStruct = newStructureSelection 
                ? structures.find(s => s.id.toString() === newStructureSelection)
                : null;
              ...
              const updatedAssignments = assignments.map((a) =>
                a.id === employeeToChange.id
                  ? {
                      ...
                      // Current Assignment
                      currentStructure: newStruct?.name || employeeToChange.currentStructure,
                      ...
                      takeHome: newStruct ? calculateBreakdown(newStruct)?.takeHome : employeeToChange.takeHome,
                      grossSalary: newStruct ? calculateBreakdown(newStruct)?.gross : employeeToChange.grossSalary,
                      effectiveDate: newEffectiveDate || employeeToChange.effectiveDate,
                      assignmentType: employeeToChange.assignmentType,
                      previousStructure: employeeToChange.previousStructure,
                      ...
                      changeReason: customReason || changeReason || employeeToChange.changeReason,
                      lastModified: new Date().toISOString().split("T")[0],
                      lastModifiedBy: "Current User",
                    }
                  : a
      ...
      <div className="modal-content">
              ...
              <div className="row">
                ...
                <div className="col-md-3 mb-3">
                  ...
                  <input
                    ...
                    readOnly
                  />
                  <div className="form-text">Auto-filled from structure</div>
                </div>
                <div className="col-md-3 mb-3">
              ...
              <div className="row">
                ...
                <div className="col-md-4 mb-3 d-flex align-items-end">
                  <div className="form-check form-switch">
                    ...
                    />
                    <label className="form-check-label" htmlFor="proRataSwitch">
                      Pro-rata Calculation
                    </label>
                  </div>
                </div>
                <div className="col-md-6 mb-3">
                  <div className="form-check form-switch">
                    <input
                      ...
                      checked={newAssignment.allocationRules.hasCustomRules}
                      onChange={(e) =>
                        setNewAssignment({
                          ...newAssignment,
                          allocationRules: {
                    ...
                    />
                    <label className="form-check-label" htmlFor="customRulesSwitch">
                      Apply Custom Rules
                    </label>
                  </div>
                </div>
                ...
                <div className="card border">
                  <div className="card-body">
                    ...
                    <div className="row">
                      <div className="col-md-6">
                        <div className="mb-2">
                          <small className="text-muted">Selected Structure</small>
                          <div className="fw-medium">
                            {structures.find((s) => s.id.toString() === newAssignment.structureId)?.name || "N/A"}
                          </div>
                        </div>
                        <div className="mb-2">
                          <small className="text-muted">Grade & Level</small>
                          <div className="fw-medium">
                            Grade {newAssignment.grade} | {newAssignment.level}
                          </div>
                        </div>
                        <div className="mb-2">
                          <small className="text-muted">Annual CTC</small>
                          <div className="fw-bold text-primary">
                            ₹{parseInt(newAssignment.ctc || 0).toLocaleString()}
                          </div>
                        </div>
                      </div>
                      <div className="col-md-6">
                        <div className="mb-2">
                          <small className="text-muted">Estimated Take-Home</small>
                          <div className="fw-bold text-success">
                            {(() => {
                              const selectedStructure = structures.find(
                        ...
                        <div className="mb-2">
                          <small className="text-muted">Assignment Type</small>
                          <div className="fw-medium">
                            {newAssignment.assignmentType.charAt(0).toUpperCase() + 
        ...
        <div className="modal-footer">
          ...
          <button
            ...
            onClick={() => {
              // Validate required fields
              if (!newAssignment.employeeId || !newAssignment.name || !newAssignment.structureId || !newAssignment.ctc) {
                setActionNotification({
                  show: true,
                  type: "error",
                  title: "Validation Error",
